...
function hexOrBuffer(valueInput, name) {
  ...
  if (!Buffer.isBuffer(value)) {
    if (!isHexString(value)) {
      const error = new Error(name ? (`[fourtwozerojs-abi] invalid ${name}`) : '[fourtwozerojs-abi] invalid hex or buffer, must be a prefixed alphanumeric even length hex string');
...
function getKeys(params, key, allowEmpty) {
  ...
  if (!Array.isArray(params)) { throw new Error(`[fourtwozerojs-abi] while getting keys, invalid params value ${JSON.stringify(params)}`); }
...
function coderFixedBytes(length) {
  return {
    ...
    decode: function decodeFixedBytes(data, offset) {
      if (data.length !== 0 && data.length < offset + 32) { throw new Error(`[fourtwozerojs-abi] while decoding fixed bytes, invalid bytes data length: ${length}`); }
...
const coderAddress = {
  encode: function encodeAddress(valueInput) {
    var value = valueInput; // eslint-disable-line
    var result = new Buffer(32); // eslint-disable-line
    if (!isHexString(value, 20)) { throw new Error('[fourtwozerojs-abi] while encoding address, invalid address value, not alphanumeric 20 byte hex string'); }
    value = hexOrBuffer(value);
    result.fill(0);
  ...
  decode: function decodeAddress(data, offset) {
    ...
    if (data.length !== 0 && data.length < offset + 32) { throw new Error(`[fourtwozerojs-abi] while decoding address data, invalid address data, invalid byte length ${data.length}`); }
    return {
      consumed: 32,
...
function decodeDynamicBytesHelper(data, offset) {
  if (data.length !== 0 && data.length < offset + 32) { throw new Error(`[fourtwozerojs-abi] while decoding dynamic bytes data, invalid bytes length: ${data.length} should be less than ${offset + 32}`); }

  var length = uint256Coder.decode(data, offset).value; // eslint-disable-line
  length = length.toNumber();
  if (data.length !== 0 && data.length < offset + 32 + length) { throw new Error(`[fourtwozerojs-abi] while decoding dynamic bytes data, invalid bytes length: ${data.length} should be less than ${offset + 32 + length}`); }
...
function coderArray(coder, lengthInput) {
  return {
    encode: function encodeArray(value) {
      ...
      if (!Array.isArray(value)) { throw new Error('[fourtwozerojs-abi] while encoding array, invalid array data, not type Object (Array)'); }
      ...
      if (length !== value.length) { throw new Error(`[fourtwozerojs-abi] while encoding array, size mismatch array length ${length} does not equal ${value.length}`); }
...
function getParamCoder(typeInput) {
  ...
  const invalidTypeErrorMessage = `[fourtwozerojs-abi] while getting param coder (getParamCoder) type value ${JSON.stringify(type)} is either invalid or unsupported by fourtwozerojs-abi.`;
